...
class GenNet(nn.Module):
    def __init__(self, numBlocks = 6):
        ...
        outLayers = [  nn.Conv2d(512, 2 * 1024, 5, stride = 1, padding = 2),
...
class MaskCycleGANVC(nn.Module):
    def __init__(self, IS_GPU = False, n_blocks = 6):
    ...
    def train(self, aLoader, bLoader, lam = 10.0, learnRate = 0.002, useIDLoss = False, NUM_EPOCHS = 20, gan_criterion = 'MSE'):
        params = list(self.agen.parameters()) + list(self.bgen.parameters())
        optimizerGen = torch.optim.Adam(params, lr=learnRate, betas=(0.5, 0.999))
        optimizerADisc = torch.optim.Adam(self.aNet.parameters(), lr = learnRate / 2, betas=(0.5, 0.999))
        optimizerBDisc = torch.optim.Adam(self.bNet.parameters(), lr = learnRate / 2, betas=(0.5, 0.999))
        optimizerA2Disc = torch.optim.Adam(self.a2Net.parameters(), lr = learnRate / 2, betas=(0.5, 0.999))
        optimizerB2Disc = torch.optim.Adam(self.b2Net.parameters(), lr = learnRate / 2, betas=(0.5, 0.999))
        ...
        for epoch in range(NUM_EPOCHS):  # loop over the dataset multiple times
            ...
            for i, data in enumerate(bLoader, 0):
                ...
                stackA[:,:,:,maskStartA: (maskStartA + 20)] = 0.
